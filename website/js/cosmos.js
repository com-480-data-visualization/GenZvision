 ...
 * Sentiment × Intensity constellation scatter plot.
 *
...
function renderCosmos() {
    ...
    d3.json('data/slang_cosmos.json').then(({ meta, constellations, terms }) => {
        ...
        // Prim's MST in pixel space — produces cleaner star patterns than sequential edges
        ...
        // For each constellation, the top star (highest intensity = topmost on screen)
        const topTermByCat = {};
        constellations.forEach(c => {
            const catTerms = terms.filter(t => t.category === c.id);
        ...
        /* ── BACKGROUND ────────────────────────────────────────────── */
        mainG.append('rect').attr('width', iW).attr('height', iH).attr('fill', '#06060f').attr('rx', 6);
        ...
        const hGridSel = gridG.selectAll('.hgrid').data(yGridVals).enter()
            ...
            .attr('stroke', 'rgba(255,255,255,0.035)').attr('stroke-dasharray', '3,5');

        const vGridSel = gridG.selectAll('.vgrid').data(xGridVals).enter()
            .append('line').attr('class', 'vgrid').attr('y1', 0).attr('y2', iH)
            .attr('x1', d => xScale(d)).attr('x2', d => xScale(d))
            .attr('stroke', d => d === 0 ? 'rgba(255,255,255,0.18)' : 'rgba(255,255,255,0.035)')
            .attr('stroke-width', d => d === 0 ? 1.5 : 0.75)
        ...
        const neutralLabel = mainG.append('text')
            ...
            .attr('fill', 'rgba(255,255,255,0.22)').attr('letter-spacing', '0.5px')
            .text('NEUTRAL');
        ...
        [
            ...
            qG.append('text')
                ...
                .attr('fill', 'rgba(255,255,255,0.06)').attr('letter-spacing', '1.5px')
        ...
        const lineSel = linesG.selectAll('.const-line').data(edgeData).enter()
            ...
            .attr('stroke-width', 1).attr('stroke-opacity', 0.28).attr('stroke-dasharray', '3,5');
        ...
        const starSel = starsG.selectAll('.star').data(terms).enter()
            ...
            .attr('r', 0)
            .attr('fill', d => colorMap[d.category] || '#fff').attr('fill-opacity', 0.88)
            .attr('stroke', d => colorMap[d.category] || '#fff')
            .attr('stroke-width', 0.6).attr('stroke-opacity', 0.35)
        ...
        const centSel  = centG.selectAll('.centroid-label').data(centData).enter()
            ...
            .attr('fill', d => d.color).attr('fill-opacity', 0.5).attr('letter-spacing', '1.5px')
        ...
        const styleAxis = g => {
            ...
            g.selectAll('.tick text').attr('fill', 'rgba(255,255,255,0.38)')
        ...
        function highlightCat(cat) {
            // Skip if this category is already dimmed by an active selection
        ...
        function resetHighlight() {
            // Restore to whatever the current selection state dictates
            const hasSel = selectedCats.size > 0;
            starSel
                .attr('fill-opacity', d => hasSel ? (selectedCats.has(d.category) ? 0.92 : 0.06) : 0.88)
                .attr('filter',       d => hasSel && !selectedCats.has(d.category) ? 'none' : 'url(#star-glow)');
            lineSel.attr('stroke-opacity', d => hasSel ? (selectedCats.has(d.catId) ? 0.6 : 0.04) : 0.28);
            centSel.attr('fill-opacity',   d => hasSel ? (selectedCats.has(d.id)  ? 0.9 : 0.04) : 0.5);
        ...
        function applyView() {
            ...
            lineSel.transition(tr)
                .attr('x1', d => xScale(d.source.sentiment)).attr('y1', d => yScale(d.source.intensity))
                .attr('x2', d => xScale(d.target.sentiment)).attr('y2', d => yScale(d.target.intensity))
                .attr('stroke-opacity', d => hasSel ? (selectedCats.has(d.catId) ? 0.6 : 0.04) : 0.28);

            centSel.transition(tr)
                .attr('x', d => xScale(d.centroid.x))
                .attr('y', centLabelY)
                .attr('fill-opacity', d => hasSel ? (selectedCats.has(d.id) ? 0.9 : 0.04) : 0.5);
        ...
        starSel
            ...
            .on('mouseleave', function(event, d) {
                ...
                d3.select(this)
                    ...
                    .attr('fill-opacity', isDimmed ? 0.06 : (selectedCats.has(d.category) ? 0.92 : 0.88));
                hoverLabelSel.attr('fill-opacity', 0);
                tooltip.style('display', 'none').style('opacity', 0);

                // Restore legend to current selection state
                if (legendEl) {
                    const hasSel = selectedCats.size > 0;
                    legendEl.querySelectorAll('.cosmos-legend-item').forEach(el => {
                        const isSel = selectedCats.has(el.dataset.cat);
        ...
        const totalDelay = 60 + byUsage.length * 28;
        setTimeout(() => { linesG.transition().duration(700).ease(d3.easeCubicOut).attr('opacity', 1); }, totalDelay + 100);
        setTimeout(() => { centG.transition().duration(500).ease(d3.easeCubicOut).attr('opacity', 1); }, totalDelay + 600);
