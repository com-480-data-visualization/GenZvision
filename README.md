...
**Live website:** [https://com-480-data-visualization.github.io/GenZvision/website/](https://com-480-data-visualization.github.io/GenZvision/website/)

**Run locally:**

```bash
git clone https://github.com/com-480-data-visualization/GenZvision.git
cd GenZvision/website
python -m http.server 8080
```

Then open [http://localhost:8080](http://localhost:8080) in a modern browser (Chrome, Firefox, or Safari recommended).

**No build step required** — the project uses vanilla JavaScript, D3.js v7, and CSS. All dependencies are loaded from CDNs:
- [D3.js v7](https://d3js.org/) — core visualization library
- [d3-sankey](https://github.com/d3/d3-sankey) — for the platform flow diagram
- [topojson-client](https://github.com/topojson/topojson-client) — for geographic projections (US + world maps)
...
GenZvision/
├── website/
│   ├── index.html              # Main page — scrollytelling layout with 11 sections
│   ├── css/
│   │   └── style.css           # All styles, animations, responsive breakpoints
│   ├── js/
│   │   ├── main.js             # Scroll observer, section transitions, hero viz
│   │   ├── timeline.js         # The Rise — monthly heatmap
│   │   ├── race.js             # The Race — bar chart race animation
│   │   ├── sankey.js           # The Spread — platform flow Sankey diagram
│   │   ├── bubbles.js          # Peak & Decline — animated lifecycle bubbles
│   │   ├── vibewheel.js        # The Vibes — context wheel with age/sentiment
│   │   ├── fresco.js           # The Chronicle — birth timeline of slang terms
│   │   ├── cosmos.js           # The Cosmos — galaxy-themed sentiment scatter
│   │   ├── regional.js         # The Map — US choropleth
│   │   ├── agegap.js           # The Generation Gap — animated donut + particles
│   │   ├── explore_regional.js # Explore — US regional deep-dive with click panel
│   │   ├── explore_world.js    # Explore — world map with country detail panel
│   │   └── explore_term.js     # Explore — individual term biography cards
│   └── data/
│       ├── slang_timeline.json       # Monthly usage counts per term
│       ├── platform_flow.json        # Origin → usage platform flows
│       ├── slang_lifecycle.json      # Term lifecycle phases over time
│       ├── top_terms_summary.json    # Aggregated term stats
│       ├── vibe_wheel_data.json      # Context × term × sentiment matrix
│       ├── fresco_data.json          # Term birth dates and origins
│       ├── slang_cosmos.json         # Sentiment, intensity, category per term
│       ├── regional_data.json        # US state and world region usage
│       └── age_data.json             # Age group distributions per term
├── data/
│   ├── sketches/                     # Hand-drawn M1/M2 sketches
│   └── *.json                        # Raw preprocessed data files
├── process_book.pdf                  # Process book (see below)
└── README.md                         # This file
```

### Intended Usage

GenZvision is a **scrollytelling data story** — the user simply scrolls down the page. Each section is revealed with a fade-in animation and its visualization renders on first view (lazy loading via Intersection Observer). The narrative flows through 11 sections:

1. **Hero** — Animated floating word cloud sets the tone
2. **The Rise** — Heatmap of monthly usage intensity for the top 15 terms
3. **The Race** — Bar chart race: hit play to watch terms compete for #1 over 5 years
4. **The Spread** — Sankey diagram showing how terms migrate between platforms
5. **Peak & Decline** — Animated bubble chart with lifecycle phases (play/pause + slider)
6. **The Vibes** — Wheel showing slang usage across 17 real-world contexts
7. **The Chronicle** — Horizontal timeline of when each term was first coined
8. **The Cosmos** — Galaxy-themed scatter plot of sentiment vs. intensity
9. **The Map** — US choropleth showing regional slang adoption
10. **The Generation Gap** — Animated donut chart with orbiting particles showing age distributions
11. **Explore** — Free-form dashboard: US map, world map with click-to-explore, and term explorer

Every visualization supports hover tooltips with detailed information. Animated sections (Race, Peak & Decline) include play/pause controls and a timeline slider. The Explore section and Generation Gap support dropdown filtering and click-to-select interactions.

### What Changed Since Milestone 2

We delivered **all 7 core MVP items** and **all 6 stretch goals**, plus 4 additional visualizations that weren't in the original plan:

| Status | Visualization | Notes |
|--------|--------------|-------|
| ✅ Core | Scrollytelling layout | 11 sections with scroll-triggered transitions and narrative connectors |
| ✅ Core | Timeline heatmap | Fully interactive with tooltips |
| ✅ Core | Platform Sankey | Hover-highlighted flows with term details |
| ✅ Core | Bubble lifecycle chart | Animated with play/pause and time slider |
| ✅ Core | Tooltips everywhere | Every data point has hover details |
| ✅ Core | Hero word cloud | D3 force simulation with floating terms |
| ✅ Core | Responsive layout | Desktop-first with mobile breakpoints |
| ✅ Stretch | Bar chart race | Ranked competition with play button |
| ✅ Stretch | Slang Cosmos | Galaxy-themed with spiral arm particle systems |
| ✅ Stretch | US choropleth | Click-to-explore with state detail panels |
| ✅ Stretch | Gen Z micro-interactions | Glow effects, spring easing, gradient text, animated counters |
| ✅ **New** | Vibe Wheel | Context-based usage wheel (17 categories) with age/sentiment rings |
| ✅ **New** | The Chronicle | Horizontal birth timeline of all 46 terms |
| ✅ **New** | World Map | Global choropleth with click-to-explore country panels |
| ✅ **New** | Generation Gap | Animated donut + particle swarm for age demographics |
| ✅ **New** | Term Explorer | Individual term biography with usage curve and stats |

**Dropped:** The ridgeline plot was replaced by the Vibe Wheel, which we felt was more visually interesting and showed context-specific patterns instead of just sentiment distributions. Cross-view Crossfilter was replaced by per-section interactivity (dropdowns, click-to-select), which felt more natural in a scrollytelling format.
